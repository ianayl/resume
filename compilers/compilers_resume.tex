...
\begin{itemize}
  ...
  \item Devising \textbf{MLIR} passes for progressively lowering a CFG to MLIR \texttt{GPU} dialect to generate/serialize GPU binaries/IR
  % \item Working on rewrite using a \textbf{LR} parser with integrated lexer, a compiler to \textbf{LLVM} IR, and a strong typing system
...
%\vspace{-0.5em}
\begin{itemize}
  \item Created an interpreted language using a \textbf{tree-walk evaluator} that processes \textbf{AST}s
  \item Devised a table-based lexer generator, and wrote a \textbf{recursive descent parser} following LL(k) grammar
  % \item Working on rewrite using a \textbf{LR} parser with integrated lexer, a compiler to \textbf{LLVM} IR, and a strong typing system
        % TODO phrasing here sucks balls redo it
  %\item Working on a static type-checker over the AST, and enabling JIT compilation using LLVM
\end{itemize}
